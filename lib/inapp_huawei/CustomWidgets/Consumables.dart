...
class Consumables extends StatefulWidget {
  final Function pushCoin;
  final Function pushBuyCoin;

  const Consumables(
      {Key? key, required this.pushCoin, required this.pushBuyCoin})
      : super(key: key);
  @override
...
class _ConsumablesState extends State<Consumables> {
  ...
  int findSTT(String id) {
    for (int i = 0; i < idConsumable.length; ++i) {
      if (id == idConsumable[i]) {
        log(i.toString());
        return i + 1;
      }
    }
  ...
  buyProduct(String productID) async {
    try {
      ...
      if (result.returnCode == HmsIapResults.ORDER_STATE_SUCCESS.resultCode) {
        int stt = findSTT(productID);
        widget.pushBuyCoin(stt);
        widget.pushCoin(stt);
        loadProducts();
        ownedPurchases();
      } else {
    ...
    try {
      ...
      if (result.inAppPurchaseDataList != null) {
        for (int i = 0; i < result.inAppPurchaseDataList!.length; i++) {
          consumeItem(result.inAppPurchaseDataList![i].purchaseToken!);
        }
      }
      setState(() {
        purchased.clear();
